...
/** Builds a "stacked range" formula fragment across all 4 department sheets, e.g. {A!B2:B;C!B2:B;...}. */
function stackedRange_(colLetter) {
  return '{' + DEPARTMENT_PROFILES.map(function (p) {
    return p.sheetName + '!' + colLetter + '2:' + colLetter;
  }).join(';') + '}';
}
...
function buildDashboard_(ss) {
  ...
  d.getRange('A1').setValue('OPS DASHBOARD — 4 Departments').setFontSize(16).setFontWeight('bold');
  d.getRange('A2').setValue('Anyone not listed under Red Flags is clear (green) today.').setFontStyle('italic');

  // ---- Today's Red Flags ---- (Flags sheet is already unified across all 4 forms)
  ...
  // ---- Submission tracker ---- (union across all 4 department response sheets)
  var namesStacked = stackedRange_('B');
  var datesStacked = stackedRange_('R');
  ...
  d.getRange('A33').setFormula(
    ...
    namesStacked + ',' + datesStacked + '=TODAY()))),0))&' +
    '" of "&COUNTA(' + SHEET_EMPLOYEES + '!A2:A)'
  );
  d.getRange('A34').setValue('Missing today:');
  d.getRange('A35').setFormula(
    '=IFERROR(FILTER(' + SHEET_EMPLOYEES + '!A2:A,ISNA(MATCH(' + SHEET_EMPLOYEES + '!A2:A,' +
    'IFERROR(FILTER(' + namesStacked + ',' + datesStacked + '=TODAY()),{"—NONE—"}),0))),' +
